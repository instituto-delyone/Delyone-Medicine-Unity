...
function findPdfTextField(form, candidates){
  ...
  const normalized = fields.map(f => ({field:f, name:f.getName(), norm:normalizeName(f.getName())}));

  for(const c of candidates){
    if(byName.has(c)){
      try { return form.getTextField(c); } catch {}
    }
  }
  ...
  for(const c of candidates){
    ...
    if(exact){
      try { return form.getTextField(exact.name); } catch {}
    }
  }
  ...
  for(const c of candidates){
    const nc = normalizeName(c);
    if(!nc) continue;
    const partial = normalized.find(x => x.norm.includes(nc) || nc.includes(x.norm));
    if(partial){
      try { return form.getTextField(partial.name); } catch {}
    }
  }
...
async function generatePdf(){
  try{
    ...
    for(const entry of activeFieldMap.fields || []){
      const value = formatValue(entry.canonicalId, data[entry.canonicalId]);
      if(!value) continue;
...
$("inspectBtn").addEventListener("click", inspectPdf);
