...
async function waitForGlobal(globalName, label, fallbackUrls=[]){
  ...
  for(const url of fallbackUrls){
    try{
      setStatus(`Carregando dependência ${label}...`, "warn");
      await loadScriptOnce(url, globalName);
      for(let i=0;i<20;i++){
        await new Promise(r => setTimeout(r, 150));
        if(window[globalName]) return true;
      }
  ...
  throw new Error(`${label} não carregou. Verifique conexão/CDN ou inclua a biblioteca no index.html.`);
}

async function waitForPDFLib(){
  return waitForGlobal("PDFLib", "pdf-lib", [
...
async function waitForJSZip(){
  return waitForGlobal("JSZip", "JSZip", [
